...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    if (access(USB_KEY_PATH, F_OK) != 0) {
        ...
        syslog(LOG_WARNING, "Authentication failed: Physical USB key missing.");
        closelog();
    ...
    snprintf(panic_pin, sizeof(panic_pin), "%s%s", real_pin, PANIC_SUFFIX);
    ...
    if (strcmp(user_input, real_pin) == 0) {
        ...
        syslog(LOG_EMERG, "DURESS CODE ENTERED! Initiating emergency wipe protocol.");
        closelog();
        
        char command[256];
        snprintf(command, sizeof(command), "rm -rf %s", SECRET_DATA_DIR);
        system(command);
        return PAM_SUCCESS;
    }
